...
@router.post("/isi-kelas")
def isi_kelas_production(key: str, db: Session = Depends(get_db)):
    if not SETUP_KEY or key != SETUP_KEY:
        raise HTTPException(403, "Kunci tidak valid")
...
def add_jadwal_active_column(key: str, db: Session = Depends(get_db)):
    ...
    try:
        db.execute(text(
            "ALTER TABLE jadwal_ruangan ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE"
        ))
        db.commit()
